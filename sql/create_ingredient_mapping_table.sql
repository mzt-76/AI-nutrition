-- Ingredient Mapping Table for OpenFoodFacts Caching
-- Purpose: Cache ingredient → OpenFoodFacts product mappings to reduce lookups and improve performance
...
CREATE TABLE IF NOT EXISTS ingredient_mapping (
    ...
    openfoodfacts_code TEXT NOT NULL,  -- OpenFoodFacts barcode/ID for the matched product
    openfoodfacts_name TEXT NOT NULL,  -- Official OpenFoodFacts product name
...
COMMENT ON TABLE ingredient_mapping IS 'Caches ingredient → OpenFoodFacts mappings to reduce lookups and improve meal plan generation performance';
