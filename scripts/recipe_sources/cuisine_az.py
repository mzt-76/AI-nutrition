"""Cuisine AZ scraping adapter for recipe import pipeline.

LLM-free (rule 10). Scrapes recipes from cuisineaz.com using BeautifulSoup.
...
CUISINEAZ_BASE = "https://www.cuisineaz.com"
USER_AGENT = (
...
class CuisineAZSource(RecipeSource):
    """Scrape recipes from CuisineAZ.com."""
    ...
    async def fetch_recipes(
        ...
        **filters: str | int | float,
        ...
        """Fetch recipes from CuisineAZ via search + page scraping.
        ...
        """
        query = str(filters.get("query", "recette fitness protéinée"))
        ...
        try:
            resp = await client.get(
                f"{CUISINEAZ_BASE}/recherche/recettes",
                params={"q": query},
                headers=headers,
            )
            resp.raise_for_status()
        except httpx.HTTPError as e:
            logger.error(f"CuisineAZ search failed: {e}")
            return []
        ...
        for a in soup.select("a[href*='/recettes/']"):
            href = a.get("href", "")
            if href and href not in links and "/recettes/" in href:
                full = href if href.startswith("http") else f"{CUISINEAZ_BASE}{href}"
                links.append(full)
            if len(links) >= limit:
                break

        logger.info(f"CuisineAZ: found {len(links)} recipe links for '{query}'")
        ...
        for url in links[:limit]:
            try:
                ...
                logger.warning(f"CuisineAZ scrape error for {url}: {e}")
    ...
    async def _scrape_recipe(
        ...
        """Scrape a single recipe page from CuisineAZ."""
    ...
    def _parse_ingredients(self, soup: BeautifulSoup) -> list[dict]:
        """Extract ingredients from a CuisineAZ recipe page."""
        ingredients: list[dict] = []

        # CuisineAZ uses structured ingredient elements
        ing_items = soup.select(".ingredient_li") or soup.select(
            "[class*='ingredient'] li"
        )
        ...
        if not ing_items:
            ...
            if ing_section:
                parent = ing_section.find_parent(["div", "section", "ul"])
                if parent:
    ...
    def _parse_ingredient_text(self, text: str) -> dict | None:
        """Parse a French ingredient text."""
        text = text.strip()
        if not text:
            return None
    ...
    def _parse_instructions(self, soup: BeautifulSoup) -> str:
        """Extract instructions from a CuisineAZ recipe page."""
        steps: list[str] = []
        step_items = soup.select(".recipe-step p") or soup.select("[class*='step'] p")
    ...
    def _infer_meal_type(self, title: str) -> str:
        ...
        snack = ["collation", "snack", "goûter", "smoothie"]
