...
import json
from nutrition.meal_plan_optimizer import round_quantity_smart
...
def build_meal_plan_prompt(
    ...
    structure_info = MEAL_STRUCTURES.get(
        meal_structure, MEAL_STRUCTURES["3_meals_2_snacks"]
    )
    ...
    protein_distribution = "\n".join(
        [f"   - {meal}: ~{protein_per_main}g protein" for meal in main_meals]
    )
    if snacks:
        protein_distribution += "\n" + "\n".join(
            [f"   - {meal}: ~{protein_per_snack}g protein" for meal in snacks]
        )

    calorie_distribution = "\n".join(
        [f"   - {meal}: ~{calorie_per_main} kcal" for meal in main_meals]
    )
    if snacks:
        calorie_distribution += "\n" + "\n".join(
            [f"   - {meal}: ~{calorie_per_snack} kcal" for meal in snacks]
        )
...
def build_meal_plan_prompt_simple(
    ...
    """
    Build simplified prompt (~100 lines) for GPT-4o meal plan generation.

    This simplified version focuses on CREATIVITY and ALLERGEN AWARENESS,
    delegating macro calculations entirely to Python. Key changes:
    - No macro calculation instructions (Python handles this)
    - Focus on recipe creativity and variety
    - Allergens mentioned 3+ times for safety
    - Concrete example at start for clarity
    - Temperature 0.8 recommended for variety

    Args:
        profile: User profile dict with allergies, targets, preferences
        meal_macros_distribution: Result from calculate_meal_macros_distribution()
            with structure:
            {
                "meals": [{"meal_type": str, "time": str, "target_calories": int, ...}],
                "daily_totals": {"calories": int, "protein_g": int, ...}
            }
        rag_context: Scientific context from RAG retrieval (truncated to 1000 chars)
        start_date: Start date in YYYY-MM-DD format

    Returns:
        Simplified prompt string for LLM (~100 lines vs 500)

    Example:
        >>> from nutrition.meal_distribution import calculate_meal_macros_distribution
        >>> distribution = calculate_meal_macros_distribution(
        ...     3300, 174, 413, 92, "3_meals_2_snacks"
        ... )
        >>> profile = {"allergies": ["peanuts"], "target_calories": 3300}
        >>> prompt = build_meal_plan_prompt_simple(
        ...     profile, distribution, "RAG context...", "2024-12-23"
        ... )
        >>> "ALLERGIES" in prompt
        True
        >>> len(prompt.split("\\n")) < 150  # Much shorter than old prompt
        True

    References:
        Plan: refactor-meal-plan-generation-workflow.md
        Temperature: 0.8 (vs 0.7) for more recipe variety
    """

    # Helper to ensure list fields
    def ensure_list(value):
        if value is None:
            return []
        if isinstance(value, list):
    ...
    allergies_list = ensure_list(profile.get("allergies"))
    allergies_str = ", ".join(allergies_list) if allergies_list else "AUCUNE"
    disliked_str = ", ".join(ensure_list(profile.get("disliked_foods"))) or "Aucun"
    favorites_str = ", ".join(ensure_list(profile.get("favorite_foods"))) or "Aucun"
    cuisines_str = (
        ", ".join(ensure_list(profile.get("preferred_cuisines")))
        or "Française, Italienne, Asiatique"
    )
    diet_type = profile.get("diet_type", "omnivore")
    max_prep_time = profile.get("max_prep_time", 60)

    # Extract daily totals
    daily_totals = meal_macros_distribution["daily_totals"]
    daily_calories = daily_totals["calories"]
    daily_protein = daily_totals["protein_g"]

    # Build meal list with targets
    meals_info = meal_macros_distribution["meals"]
    meals_list_text = "\n".join(
        [
            f"   - {m['meal_type']} ({m['time']}): ~{m['target_calories']} kcal, ~{m['target_protein_g']}g protéines"
            for m in meals_info
        ]
    )
    ...
    # Allergen warnings (mentioned 3x)
    allergen_warning = ""
    if allergies_list:
        allergen_warning = f"""
🚨 ALLERGIES CRITIQUES : {allergies_str}
⚠️  TOLÉRANCE ZÉRO - Vérifier CHAQUE ingrédient avant inclusion
⚠️  Si allergie détectée → REMPLACER l'ingrédient, NE PAS inclure le repas
"""

    # Truncate RAG context
    rag_truncated = rag_context[:1000] if len(rag_context) > 1000 else rag_context

    # Build simplified prompt
    prompt = f"""Tu es un chef cuisinier nutritionniste expert. Génère un plan de repas hebdomadaire CRÉATIF et VARIÉ.
    ...
    {{
      ...
      "meals": [
        {{
          "meal_type": "Petit-déjeuner",
          "time": "07:30",
          "recipe_name": "Omelette protéinée aux légumes",
          "ingredients": [
            {{"name": "Eggs", "quantity": 3, "unit": "units"}},
            {{"name": "Bell pepper", "quantity": 50, "unit": "g"}},
            {{"name": "Cheese", "quantity": 30, "unit": "g"}}
          ],
          "instructions": "Battre les œufs. Couper les poivrons. Cuire dans une poêle. Ajouter le fromage."
        }}
      ]
    }}
  ]
...
IMPORTANT : Instructions en FRANÇAIS, noms d'ingrédients en ANGLAIS.

═══════════════════════════════════════════════════════════════
PROFIL UTILISATEUR
═══════════════════════════════════════════════════════════════

Objectif quotidien : {daily_calories} kcal | {daily_protein}g protéines
{allergen_warning}
Aliments détestés : {disliked_str}
Aliments favoris : {favorites_str}
Cuisines préférées : {cuisines_str}
Régime alimentaire : {diet_type}
Temps de préparation max : {max_prep_time} min

═══════════════════════════════════════════════════════════════
REPAS QUOTIDIENS (avec cibles approximatives)
═══════════════════════════════════════════════════════════════

{meals_list_text}

⚠️ IMPORTANT : Ces cibles de macros sont OBLIGATOIRES et basées sur le profil utilisateur.
Respecte les quantités de protéines par repas indiquées ci-dessus. Le système Python optimisera les portions finales.
...
1. VARIÉTÉ : Chaque jour différent (ne répète PAS les mêmes recettes)
2. CRÉATIVITÉ : Recettes originales inspirées de {cuisines_str}
3. ALLERGIES : {f"EXCLURE ABSOLUMENT {allergies_str}" if allergies_list else "Aucune restriction"}
4. INGRÉDIENTS : Noms en ANGLAIS (ex: "Chicken breast", "Rice", "Broccoli")
5. INSTRUCTIONS : En FRANÇAIS, 1 seule chaîne, étapes séparées par des points
6. QUANTITÉS : Respecte les cibles protéines par repas listées ci-dessus (basées sur le profil utilisateur)
7. SIMPLICITÉ : Recettes faisables en < {max_prep_time} min
...
{{
  ...
  "days": [
    {{
      ...
      "meals": [
        {{
          "meal_type": "Nom du repas",
          "time": "HH:MM",
          "recipe_name": "Nom créatif",
          "ingredients": [{{"name": "ENGLISH_NAME", "quantity": number, "unit": "g/ml/units"}}],
...
- "meal_plan_id": "plan_{start_date}" (EXACTEMENT ce format)
- "start_date": "{start_date}" (date fournie)
- "days": tableau de 7 jours

⚠️  DERNIÈRE VÉRIFICATION AVANT ENVOI :
- meal_plan_id et start_date présents à la RACINE du JSON ? ✓
- Allergies {allergies_str} = ABSENTES de TOUS les ingrédients ? ✓
- 7 jours complets ? ✓
- Recettes variées (pas de répétitions) ? ✓
- Ingrédients en ANGLAIS ? ✓
- Instructions en FRANÇAIS ? ✓

Génère maintenant le plan complet pour les 7 jours !
"""

    logger.info(
        f"Built simplified prompt: ~{len(prompt.split(chr(10)))} lines "
        f"(daily: {daily_calories} kcal, {daily_protein}g protein, "
        f"{len(meals_info)} meals/day, allergies: {allergies_str})"
    )
